...
For a requirements only setup, use `pip install -r requirements.txt`. Optional Torch acceleration can be installed with `pip install -e ".[torch]"`.

## License

Original code in this repository is licensed under the **Apache License 2.0**; see [`LICENSE`](LICENSE). The bundled CEC-2013 LSGO code under `src/evo_subspace/problems/cec2013lsgo/` remains under **GNU GPLv3**; see [`src/evo_subspace/problems/cec2013lsgo/LICENSE`](src/evo_subspace/problems/cec2013lsgo/LICENSE).
...
### Single-phase (`evo-subspace`)

Classic approach: one subspace (or full space) for the entire NFE budget.

```bash
evo-subspace --problem cec2013_lsgo_f1 --dim 1000 --subspace_method random_projection --subspace_dim 100 --subspace_assignment absolute --optimizer de --pop_size 100 --init_pop uniform --de_mut_rate 0.8 --de_cr_rate 0.9 --max_nfe 3000000 --seed 0 --benchmark_seed 0
```

Full-$D$ baseline (no subspace reduction; EA operates directly in $\mathbb{R}^D$):

```bash
evo-subspace --problem cec2013_lsgo_f1 --dim 1000 --subspace_method fullspace --subspace_assignment absolute --optimizer de --pop_size 100 --max_nfe 3000000 --seed 0
```

The compatibility wrapper `python scripts/main.py ...` is still available after installation.

### Two-phase (`evo-subspace-two-phase`)
...
```bash
evo-subspace-two-phase --problem cec2013_lsgo_f1 --dim 1000 --subspace_method random_projection --subspace_dim 100 --subspace_assignment additive --optimizer de --pop_size 100 --max_nfe 3000000 --full_nfe 1500000 --sub_nfe 1500000 --phase2_init from_phase1 --seed 0
```
...
Extra arguments in addition to all flags shared with `evo-subspace`:
...
```bash
evo-subspace-dual-ea --problem cec2013_lsgo_f1 --dim 1000 --subspace_method lora --lora_rank 4 --subspace_assignment additive --optimizer de --pop_size 100 --max_nfe 3000000 --seed 0 --benchmark_seed 0
```
...
|   +-- subspaces/        # Subspace maps and LoRA variants
|   +-- problems/         # CEC-2013 LSGO and GNBG support
|   +-- optimizers/       # PyMOO algorithm and sampling factories
|   +-- runtime/          # PyMOO wrapper and logging callback
+-- scripts/              # Compatibility wrappers for old CLI paths
+-- experiments/
|   +-- landscapes/       # Landscape visualization scripts
|   +-- coverage/         # Projection and subspace coverage scripts
|   +-- tables/           # Report table scripts
|   +-- synthetic/        # Synthetic LoRA studies
+-- configs/              # W&B sweep YAMLs
+-- results/
|   +-- tables/           # Generated CSV, JSON sidecars, and LaTeX tables
|   +-- figures/          # Generated figures
|   +-- caches/           # W&B and experiment caches
|   +-- archive/          # Superseded pilot outputs
...
```bash
for method in random_projection random_blocking lora; do for d in 50 100 200 500; do evo-subspace --problem cec2013_lsgo_f7 --dim 1000 --subspace_method $method --subspace_dim $d --optimizer de --pop_size 100 --max_nfe 3000000 --seed 0 --wandb --wandb_project evo-subspace-opt --wandb_group "ablation-d" --wandb_name "${method}-d${d}"; done; done
```
