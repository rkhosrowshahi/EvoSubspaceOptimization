...
``best_fitness`` over all runs that share the same simplified method label
(derived from each run's W&B ``group``), and writes a CSV with benchmark
functions as rows and methods as columns under ``results/tables/``. Missing
(group, function) pairs are written as ``--``.

When a run's configured budget equals ``--nfe``, the value is read from
``wandb.summary['best_fitness']``. When the run budget exceeds ``--nfe``, the
value at that evaluation budget is taken from the W&B history (logged with
``step=nfe``). Runs whose budget is below ``--nfe`` are skipped.

Use ``--from-local`` to skip the W&B API and regenerate LaTeX from an existing
CSV in ``--out_dir`` (see ``--input_csv``). Pair the CSV with a ``{basename}_seeds.json``
...
Outputs are written under ``results/tables/cec2013_lsgo/dim{D}/`` by default (see
...
def default_out_dir_for_dim(dim: int) -> Path:
    ...
    return RESULTS_ROOT / "tables" / "cec2013_lsgo" / f"dim{dim}"


def _fmt_nfe_latex(nfe: int) -> str:
    """Format an NFE count as a compact LaTeX math string (e.g. ``10^{6}``)."""
    if nfe <= 0:
        return str(nfe)
    exp = math.floor(math.log10(nfe))
    mantissa = nfe / (10 ** exp)
...
def _nfe_label_suffix(nfe: int) -> str:
    """Stable label suffix for nondefault NFE tables."""
    return "" if nfe == DEFAULT_NFE else f"-nfe{nfe:.3g}".replace("+", "")
...
def _run_budget_nfe(cfg: dict, summary: dict | None = None) -> int | None:
    """Configured or logged maximum NFE budget for a W&B run."""
    for key in ("max_nfe", "total_nfe"):
        v = _parse_config_int(cfg.get(key))
        if v is not None:
            return v
    if summary:
        v = _parse_config_int(summary.get("total_nfe"))
        if v is not None:
            return v
    return None


def _history_step_margin(cfg: dict) -> int:
    """Steps to scan before ``target_nfe`` (logged every ``pop_size`` evaluations)."""
    pop = _parse_config_int(cfg.get("pop_size"))
    if pop is not None and pop > 0:
        return max(pop * 2, 200)
    return 2000


def _history_scan_min_steps(target_nfe: int, cfg: dict) -> tuple[int, ...]:
    """Progressive ``min_step`` values for W&B history scans.

    DE logs at ``step=nfe`` every ``pop_size`` evaluations, so a tight window just
    below ``target_nfe`` is enough. Dual EA and other protocols can leave gaps
    (e.g. last point at 999769 with nothing in [999800, 10^6]), so we widen the
    window when the tight scan returns no rows.
    """
    margin = _history_step_margin(cfg)
    candidates = (
        max(0, target_nfe - margin),
        max(0, target_nfe - max(target_nfe // 50, margin)),
        max(0, target_nfe - max(target_nfe // 10, margin)),
        0,
    )
    seen: set[int] = set()
    out: list[int] = []
...
def _summary_fitness_at_target_nfe(
    summary: dict,
    *,
    target_nfe: int,
    fitness_key: str,
) -> float | None:
    """Use ``wandb.summary[fitness_key]`` when the run finished at ``target_nfe``.

    Training logs ``wandb.log(..., step=nfe)`` during the run and sets
    ``summary['best_fitness']`` once at the end. When the configured budget equals
    ``target_nfe``, that summary value is the terminal fitness at the checkpoint.
    """
    bf = summary.get(fitness_key)
    if bf is None:
        return None
    logged_nfe = _parse_config_int(summary.get("nfe"))
    if logged_nfe is None:
        logged_nfe = _parse_config_int(summary.get("total_nfe"))
    if logged_nfe is None:
        logged_nfe = _parse_config_int(summary.get("_step"))
    if logged_nfe is None or logged_nfe != target_nfe:
        return None
    try:
        return float(bf)
    except (TypeError, ValueError):
...
def _best_fitness_from_wandb_history(
    run,
    target_nfe: int,
    fitness_key: str,
    *,
    ...
    """Return ``best_fitness`` at the largest logged step not exceeding ``target_nfe``."""
    best_val: float | None = None
    best_step = -1
    try:
        rows = run.scan_history(
            keys=[fitness_key, "nfe", "_step"],
            min_step=min_step,
            max_step=target_nfe,
        )
    except Exception:
    ...
    for row in rows:
        ...
        try:
            ...
            continue
        if step_i > target_nfe:
            continue
        bf = row.get(fitness_key)
        if bf is None:
            continue
        try:
            val = float(bf)
...
def _best_fitness_from_wandb_run(
    run,
    *,
    target_nfe: int,
    fitness_key: str = "best_fitness",
) -> tuple[float | None, str | None]:
    """Return ``(best_fitness, skip_reason)`` for a run at evaluation budget ``target_nfe``."""
    cfg = getattr(run, "config", {}) or {}
    summary = getattr(run, "summary", {}) or {}
    run_budget = _run_budget_nfe(cfg, summary)

    if run_budget is not None and run_budget < target_nfe:
        return None, f"run budget {run_budget} < target NFE {target_nfe}"

    if run_budget is not None and run_budget == target_nfe:
        summary_val = _summary_fitness_at_target_nfe(
            summary, target_nfe=target_nfe, fitness_key=fitness_key
        )
        if summary_val is not None:
            return summary_val, None
        bf = summary.get(fitness_key)
        if bf is None:
            return None, f"missing summary key {fitness_key!r}"
        try:
            return float(bf), None
        except (TypeError, ValueError):
            return None, f"non-numeric {fitness_key}={bf!r}"
    ...
    for min_step in _history_scan_min_steps(target_nfe, cfg):
        hist_val = _best_fitness_from_wandb_history(
            run,
            target_nfe,
            fitness_key,
            min_step=min_step,
        ...
        if hist_val is not None:
            return hist_val, None

    budget_note = f" (run budget {run_budget})" if run_budget is not None else ""
    return None, f"no {fitness_key} history at NFE <= {target_nfe}{budget_note}"


def _cache_key_for_nfe(fitness_key: str, nfe: int) -> str:
    return f"{fitness_key}@{int(nfe)}"


def _jsonable(obj: object) -> object:
    """Convert W&B objects to JSON friendly primitives."""
    ...
    try:
        item = object.__getattribute__(obj, "item")
    ...
    if callable(item):
        try:
            ...
            pass
    try:
        return _jsonable(dict(obj))  # type: ignore[arg-type]
    except (TypeError, ValueError):
        return str(obj)


def default_wandb_cache_path(out_dir: Path, basename: str) -> Path:
    """Local W&B run cache used to avoid repeated API history scans."""
    try:
        rel_out_dir = out_dir.relative_to(RESULTS_ROOT / "tables")
    except ValueError:
        return out_dir / "caches" / f"{basename}_wandb_runs.jsonl"
    return RESULTS_ROOT / "caches" / rel_out_dir / f"{basename}_wandb_runs.jsonl"


def load_wandb_cache(path: Path) -> dict[str, dict[str, object]]:
    """Load cached W&B run records, keeping the newest record for each run id."""
...
def append_wandb_cache_record(path: Path, record: dict[str, object]) -> None:
    """Append one cached W&B run record so interrupted downloads can resume."""
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("a", encoding="utf-8") as f:
        f.write(json.dumps(record, sort_keys=True) + "\n")


def _cached_metric_value(
    record: dict[str, object],
    *,
    target_nfe: int,
    fitness_key: str,
) -> tuple[float | None, str | None]:
    """Read the cached best fitness value for ``target_nfe``."""
    key = _cache_key_for_nfe(fitness_key, target_nfe)
    fitness_at_nfe = record.get("fitness_at_nfe")
    if isinstance(fitness_at_nfe, dict) and key in fitness_at_nfe:
        try:
            return float(fitness_at_nfe[key]), None
        except (TypeError, ValueError):
            return None, f"non-numeric cached {key}={fitness_at_nfe[key]!r}"

    skip_reasons = record.get("skip_reasons_at_nfe")
    if isinstance(skip_reasons, dict) and key in skip_reasons:
        return None, str(skip_reasons[key])
    return None, f"missing cached {key}"


def _wandb_run_to_cache_record(
    run,
    *,
    target_nfe: int,
    fitness_key: str,
) -> dict[str, object]:
    """Download one W&B run's metadata and target NFE fitness into a cache record."""
    cfg = getattr(run, "config", {}) or {}
    summary = getattr(run, "summary", {}) or {}
    val, skip_reason = _best_fitness_from_wandb_run(
        run, target_nfe=target_nfe, fitness_key=fitness_key
    )
    key = _cache_key_for_nfe(fitness_key, target_nfe)
    record: dict[str, object] = {
        "id": str(run.id),
        ...
        "group": str(getattr(run, "group", "") or ""),
    ...
    if val is not None:
        record["fitness_at_nfe"] = {key: float(val)}
    if skip_reason is not None:
        record["skip_reasons_at_nfe"] = {key: skip_reason}
    return record
...
def write_latex_table(
    ...
    *,
    ...
    nfe_tex = _fmt_nfe_latex(nfe)
    dim_tex = r"10^{3}" if dim == 1000 else str(dim)
    label_suffix = _nfe_label_suffix(nfe)
    caption = (
        f"Mean best fitness on the CEC~2013 LSGO benchmark at $D = {dim_tex}$ after "
        f"${nfe_tex}$ function evaluations with differential evolution (DE). "
    ...
    tex = "\n".join(
        [
            ...
            "%   python3 -m experiments.tables.generate_table --dim 1000",
            r"\begin{table}[p]",
            r"\centering",
            rf"\caption{{{caption}}}",
            rf"\label{{tab:lsgo-all-fs-dim{dim}{label_suffix}}}",
            r"\scriptsize",
...
def write_latex_compact_table(
    ...
    *,
    ...
    nfe_tex = _fmt_nfe_latex(nfe)
    dim_tex = r"10^{3}" if dim == 1000 else str(dim)
    label = "tab:lsgo-preliminary" + _nfe_label_suffix(nfe)
    fn_range = (
...
def _sidecar_run_record(record: dict[str, object], best_fitness: float) -> dict[str, object]:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Average W&B best_fitness by group across selected problems and "
            "``config.dim``. Uses run summary at the full budget and W&B "
            "history at intermediate ``--nfe`` when the run budget is larger."
        )
    )
    parser.add_argument(
        "--wandb_entity",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "``results/tables/cec2013_lsgo/dim{D}/`` for the chosen ``--dim``."
        ),
    )
    parser.add_argument(
        "--basename",
    ...
    parser.add_argument(
        "--nfe",
        type=float,
        default=float(DEFAULT_NFE),
        metavar="N",
        help=(
            "Number of function evaluations reported in table captions and used "
            "to name output files when it differs from the default "
            f"({DEFAULT_NFE:,}). Accepts scientific notation (e.g. 1e6)."
        ),
    )
    parser.add_argument(
        "--fitness_key",
        ...
        metavar="KEY",
        help=(
            "W&B metric key for best fitness in summary and history "
            "(default: ``best_fitness``). Intermediate ``--nfe`` budgets read "
            "this key from history when the run's configured budget is larger."
        ),
    )
    parser.add_argument(
        "--nfe_config_key",
        ...
        metavar="KEY",
        help=(
            "Optional W&B run config key (e.g. ``max_nfe`` or ``total_nfe``). "
            "When set, only runs whose config value is at least ``--nfe`` are "
            "included. When omitted, runs are not filtered by NFE budget."
        ),
    )
    parser.add_argument(
        "--from-local",
        ...
        help=(
            ...
            "``--input_csv``). Loads ``{basename}_seeds.json`` when present "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Local JSONL cache for W&B run metadata and target NFE fitness. "
            "Defaults to ``results/caches/cec2013_lsgo/dim{D}/{basename}_wandb_runs.jsonl``."
        ),
    )
    parser.add_argument(
        "--download_wandb_cache",
        action="store_true",
        help=(
            "Fetch matching W&B runs, append or resume the local cache, then "
            "generate tables from that cache."
        ),
    )
    parser.add_argument(
        "--from_wandb_cache",
        action="store_true",
        help=(
            "Skip the W&B API and generate tables from ``--wandb_cache``. This "
            "still writes the CSV, seeds sidecar, run index, and LaTeX files."
        ),
    )
    parser.add_argument(
        "--ignore_wandb_cache",
        action="store_true",
        help=(
            "Ignore an existing W&B run cache and query the W&B API directly. "
            "Use with ``--download_wandb_cache`` to refresh cached records."
        ),
    )
    ...
    if args.nfe <= 0:
        parser.error("--nfe must be positive")
    nfe = int(round(args.nfe))
    ...
    if args.basename is None:
        ...
        nfe_suffix = "" if nfe == DEFAULT_NFE else f"_nfe{nfe:.3g}".replace("+", "")
        args.basename = f"cec2013_lsgo_{slug}_dim{args.dim}{nfe_suffix}_by_group"
    if args.wandb_cache is None:
        args.wandb_cache = default_wandb_cache_path(args.out_dir, args.basename)
    if (
        not args.from_local
        and not args.from_wandb_cache
        and not args.download_wandb_cache
        and not args.ignore_wandb_cache
        and args.wandb_cache.is_file()
    ):
        args.from_wandb_cache = True
    ...
    fitness_key: str = args.fitness_key
    nfe_config_key: str | None = args.nfe_config_key
    cache_key = _cache_key_for_nfe(fitness_key, nfe)
    run_records: list[dict[str, object]] = []

    if args.from_wandb_cache:
        cached = load_wandb_cache(args.wandb_cache)
        if not cached:
            raise FileNotFoundError(f"No W&B cache records found at {args.wandb_cache}")
        run_records = list(cached.values())
        print(f"Loaded {len(run_records)} W&B run records from cache: {args.wandb_cache}")
    else:
        ...
        cached = load_wandb_cache(args.wandb_cache) if args.download_wandb_cache else {}
        for run_idx, run in enumerate(runs, start=1):
            run_id = str(run.id)
            rec = cached.get(run_id)
            fitness_at_nfe = rec.get("fitness_at_nfe") if rec else None
            skip_reasons = rec.get("skip_reasons_at_nfe") if rec else None
            cached_metric = isinstance(fitness_at_nfe, dict) and cache_key in fitness_at_nfe
            if rec is None or not cached_metric:
                rec = _wandb_run_to_cache_record(
                    run, target_nfe=nfe, fitness_key=fitness_key
                )
                if args.download_wandb_cache:
                    append_wandb_cache_record(args.wandb_cache, rec)
                    cached[run_id] = rec
            run_records.append(rec)
            if run_idx % 25 == 0 or run_idx == len(runs):
                source = "cache/download" if args.download_wandb_cache else "API"
                print(
                    f"Prepared {run_idx}/{len(runs)} runs from {source}...",
                    flush=True,
                )
        if args.download_wandb_cache:
            print(f"W&B run cache ready: {args.wandb_cache}")
    ...
    for run_idx, run in enumerate(run_records, start=1):
        g_raw = run.get("group")
        ...
        if nfe_config_key is not None:
            run_nfe = _parse_config_int(cfg.get(nfe_config_key))
            if run_nfe is None or run_nfe < nfe:
                skipped.append(
                    f"{run.get('id', '')} ({run.get('name', '')}) "
                    f"{nfe_config_key}={run_nfe!r} < {nfe}"
                )
                continue

        val, skip_reason = _cached_metric_value(
            run, target_nfe=nfe, fitness_key=fitness_key
        )
        if skip_reason is not None or val is None:
            skipped.append(
                f"{run.get('id', '')} ({run.get('name', '')}) "
                f"{skip_reason or 'missing best_fitness'}"
            )
            continue
