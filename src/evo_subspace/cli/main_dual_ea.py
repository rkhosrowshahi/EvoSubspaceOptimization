...
See README.md for single phase (``evo-subspace``) and sequential two phase
(``evo-subspace-two-phase``) entry points.
"""
...
from evo_subspace.cli.main import (
...
from evo_subspace.cli.main_two_phase import CenteredSampling, _set_algorithm_sampling
...
def _population_checkpoint_arrays(
    algo,
    subspace,
    *,
    expand_to_full: bool,
) -> tuple[np.ndarray, np.ndarray]:
    """Return the last population coordinates and flat fitness values."""
    pop = algo.pop
    if pop is None:
        search_dim = getattr(subspace, "search_dim", 0)
        return np.empty((0, search_dim), dtype=float), np.empty((0,), dtype=float)

    X = np.asarray(pop.get("X"), dtype=float)
    F = np.asarray(pop.get("F"), dtype=float).reshape(-1)
    if expand_to_full:
        X = np.asarray(subspace.expand(X), dtype=float)
    if X.ndim == 1:
        X = X.reshape(1, -1)
    return X, F


def _log_wandb_final_checkpoint(
    *,
    best_x: np.ndarray,
    best_f: float,
    full_algo,
    ...
    """Log final solution and population arrays as a W&B artifact checkpoint."""
    import wandb  # type: ignore

    if wandb.run is None:
        return

    full_X, full_F = _population_checkpoint_arrays(
        full_algo,
        full_subspace,
        expand_to_full=True,
    )
    sub_Z, sub_F = _population_checkpoint_arrays(
        sub_algo,
        sub_subspace,
    ...
    checkpoint_path = checkpoint_dir / "dual_ea_final_checkpoint.npz"
    np.savez_compressed(
        checkpoint_path,
        global_best_x_full=np.asarray(best_x, dtype=float).reshape(-1),
        global_best_fitness=np.asarray([best_f], dtype=float),
        full_population_x_full=full_X,
        full_population_fitness=full_F,
        sub_population_z=sub_Z,
        sub_population_fitness=sub_F,
        n_cycles=np.asarray([n_cycles], dtype=np.int64),
        total_nfe=np.asarray([total_nfe], dtype=np.int64),
        elapsed_seconds=np.asarray([elapsed], dtype=float),
        full_n_eval=np.asarray([full_algo.evaluator.n_eval], dtype=np.int64),
        sub_n_eval=np.asarray([sub_algo.evaluator.n_eval], dtype=np.int64),
        full_n_gen=np.asarray([full_algo.n_gen], dtype=np.int64),
        sub_n_gen=np.asarray([sub_algo.n_gen], dtype=np.int64),
    )

    artifact = wandb.Artifact(
        name=f"dual-ea-final-checkpoint-{wandb.run.id}",
        type="checkpoint",
        metadata={
            "best_fitness": float(best_f),
            "total_nfe": int(total_nfe),
            "n_cycles": int(n_cycles),
            "full_population_shape": list(full_X.shape),
            "sub_population_shape": list(sub_Z.shape),
        },
    )
    artifact.add_file(str(checkpoint_path), name=checkpoint_path.name)
    logged_artifact = wandb.log_artifact(artifact, aliases=["final"])
